...
<template>
    ...
    >
        ...
        <el-table-column property="" label="URL" show-overflow-tooltip>
        <template #default="scope">{{ scope.row.url }}</template>
        </el-table-column>
        <el-table-column property="" label="资源类型" show-overflow-tooltip>
        <template #default="scope">{{ scope.row.resourceType }}</template>
        </el-table-column>
...
export default {
    ...
    const handleDelete = (row)=>{
        ...
        props.onRowDelete(row.resourceId)
        }
    }
    ...
    const handleSelectionChange = (selection)=>{
        ...
        state.selectedIds = selection.map(item=>item.resourceId)
    }
